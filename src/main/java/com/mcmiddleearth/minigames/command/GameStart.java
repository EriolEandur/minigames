...
public class GameStart extends AbstractGameCommand{
        ...
        if(game != null && isManager((Player) cs, game)) {
            ...
            switch(game.getType()) {
                ...
                case GOLF:
                    ...
                    break;
                case PVP:
                    if (PluginData.pvpRunning) {
                        sendPvPRunningMessage(cs);
                        return;
                    }
    ...
    private void sendPvPRunningMessage(CommandSender cs) {
        PluginData.getMessageUtil().sendErrorMessage(cs, "A pvp match is already running, try again later.");
    }
